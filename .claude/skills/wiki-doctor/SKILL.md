...
- It also flags orphan attachments (`ORPHAN ATTACHMENT CHECK`): files in the vault-root `_resources/` (Obsidian's default paste folder) that no note references and that have no companion `.md`. Report-only — with no referencing note there is no destination to relocate to; review each with the user before deleting or relocating.
