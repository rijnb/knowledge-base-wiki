...
- For relevant topics: create a new page or update an existing one.
  - Always create pages at exactly one level deep: `wiki/<topic>/<page>.md` — never deeper (e.g. `wiki/concepts/RouteSDK.md`, not `wiki/concepts/Navigation/RouteSDK.md`).
  - **Every page MUST have a one-line `description:` frontmatter field** (plain text, wikilinks allowed, ~160 chars max, YAML double-quoted) — set it when creating a page, and add it if missing when updating one. Index pages draw their bullet descriptions from it. Use `type:` values from exactly: competition, concept, conversation, decision, person, problem, project, system (never `competitor` or `systems`). Subject state goes in `state:` (e.g. `state: active`); `status:` is reserved for the optional OKF lifecycle enum `draft | stable | deprecated`.
  ...
  - **Provenance frontmatter (OKF v0.2):** when creating a page, set the provenance fields in YAML frontmatter — `generated:` with indented `by: "agent:wiki-ingest"` / `at: <evidence date, YYYY-MM-DD>` (block form, not `{...}`) and a `sources:` list (`- id: s1` / `resource: "raw/notes/2024-04-04 Foo.md"`). When updating a page, append the new raw note to `sources:` (next `s<N>` id); never remove existing `verified:` entries (`human:*` entries are the human-reviewed tier). `^block-id` anchors in the body remain allowed as plain anchors, but carry no metadata — the legacy provenance callouts are abolished; do not write them.
  - **Per-claim source footnotes (OKF v0.2):** every claim you add or update must cite its source with a markdown footnote keyed to the `sources[].id` — append `[^s<N>]` at the end of the claim text, **before** any `^block-id` anchor (the anchor must stay last in the block), e.g. `RouteSDK 1.0 launched. [^s2] ^routesdk-launch`. A claim drawn from several sources gets several refs (`[^s1][^s3]`). Maintain the footnote-definitions block at the very end of the page — one line per referenced id, `[^s<N>]: [[<resource path exactly as in frontmatter, including the .md extension>]]` — adding a definition for every new id you reference. Do not define ids never referenced in the body.
...
```json
{"date":"YYYY-MM-DD HH:mm:ss","session":1,"file":"raw/notes/filename.md","summary":"One-sentence description.","pages_created":["wiki/concepts/RouteSDK.md"],"pages_updated":["wiki/people/Jane Smith.md"]}
```
...
{"date":"YYYY-MM-DD HH:mm:ss","session":1,"file":"raw/emails/foo.md","summary":"One-sentence description.","pages_created":["wiki/concepts/RouteSDK.md"],"pages_updated":["wiki/people/Jane Smith.md"]}
```
  Note `x.md` (in batch) with a PDF attachment `y.pdf` converted on the fly (three entries):
```json
{"date":"YYYY-MM-DD HH:mm:ss","session":1,"file":"raw/notes/x.md","summary":"One-sentence description.","pages_created":[],"pages_updated":["wiki/projects/StreamKit.md"]}
